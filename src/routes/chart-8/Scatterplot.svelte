<script>
	import { cars } from '$lib/cars.js';
	import { scaleLinear, scaleThreshold } from 'd3';
	import Grid from './Grid.svelte';

	const data = cars;
	const x = (d) => d.Horsepower;
	const y = (d) => d.Acceleration;
	const color = () => 'hotpink';

	let width = 200;
	$: height = width * 0.8;

	$: xScale = scaleLinear().domain([0, 250]).range([0, width]);
	$: yScale = scaleLinear().domain([5, 30]).range([height, 0]);
	$: rScale = scaleThreshold().domain([200, 400]).range([1, 2, 4]);
</script>

<div class="chart" bind:clientWidth={width} style:height={`${height}px`}>
	<svg {height} {width} style:position="absolute" viewBox={`0 0 ${width} ${height}`}>
		{#each data as datum}
			<circle cx={xScale(x(datum))} cy={yScale(y(datum))} r={rScale(width)} fill={color(datum)} />
		{/each}
	</svg>

	<div style:position="absolute" style:width={`${width}px`} style:height={`${height}px`}>
		<Grid vertical scale={xScale} count={5} />
		<Grid horizontal scale={yScale} count={5} />
	</div>
</div>

<style>
</style>
